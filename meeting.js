...
class MeetingRoom {
    ...
    setupEventListeners() {
        ...
        // 悬浮记录条快捷键
        document.getElementById('floating-host-input')?.addEventListener('keydown', (e) => {
            if (e.key === 'Enter' && e.ctrlKey) {
                e.preventDefault();
                this.floatingAddContent();
            }
        });
...
`;
    ...
    async extractActionItems() {
        ...
        if (allContents.length === 0) {
            this.showNotification('暂无内容可提取待办', 'warning');
            return;
        }
        
        const btn = document.getElementById('extract-todo-btn');
        ...
        try {
            ...
            if (apiSettings.url && apiSettings.key && apiSettings.model) {
                // 使用AI提取待办事项
                actionItems = await this.extractTodosWithAI(allContents, apiSettings);
            } else {
                // 使用本地逻辑提取
                actionItems = this.extractTodosLocally(allContents);
            }
            
            if (actionItems.length === 0) {
                this.showNotification('未找到待办事项', 'info');
                return;
            }
            ...
            // 过滤掉重复的待办
            const existingDescriptions = new Set(existingTodos.map(t => t.description));
            const newTodos = actionItems.filter(item => !existingDescriptions.has(item.description));
            
            if (newTodos.length === 0) {
                this.showNotification('待办事项已全部存在，无需重复添加', 'info');
                return;
            }
            
            // 添加到待办列表
            existingTodos.push(...newTodos);
            localStorage.setItem('meetingTodos', JSON.stringify(existingTodos));
            
            // 同步到云端
            cloudSync.saveMeetingTodos(existingTodos);
            
            this.showNotification(`成功提取 ${newTodos.length} 个待办事项！`, 'success');
    ...
    // 使用AI提取待办事项
    async extractTodosWithAI(contents, settings) {
        const prompt = this.buildExtractTodosPrompt(contents);
        
        const systemPrompt = `你是一个专业的会议行动项提取助手。请从会议内容中提取所有待办事项。
...
请按以下JSON格式输出（只输出JSON，不要其他内容）：
{
    "actionItems": [
        {
            ...
            "assignee": "负责人（从内容中推断，未明确则填"未指定"）",
            "deadline": "截止日期（从内容中推断，未明确则填null）"
        }
    ]
}`;
        ...
        try {
            ...
            if (jsonMatch) {
                ...
                return parsed.actionItems.map((item, index) => ({
            ...
            return this.extractTodosLocally(contents);
        }
    }
    
    // 构建提取待办的prompt
    buildExtractTodosPrompt(contents) {
        const participants = [...new Set(contents.map(c => c.participant).filter(Boolean))];
        
        let prompt = `请从以下会议内容中提取所有待办事项：\n\n`;
        
        contents.forEach((c, i) => {
            if (c.content) {
                prompt += `${c.participant || '参会者'}的发言：${c.content}\n`;
            }
            if (c.thisWeek) {
                prompt += `${c.participant || '参会者'}本周事项：${c.thisWeek}\n`;
            }
            if (c.blockers) {
                prompt += `${c.participant || '参会者'}的卡点：${c.blockers}\n`;
            }
        });
    ...
    extractTodosLocally(contents) {
        ...
        contents.forEach((c, index) => {
            ...
            // 识别包含"行动项"、"TODO"的内容
            if (text.includes('行动项') || text.includes('TODO') || text.includes('待办')) {
                // 提取行动项描述
                const matches = text.match(/(?:行动项|TODO|待办)[：:]\s*(.+?)(?:\n|$)/gi);
                if (matches) {
                    matches.forEach((match, i) => {
                        const desc = match.replace(/(?:行动项|TODO|待办)[：:]\s*/i, '');
                        if (desc.trim()) {
                            todos.push({
                                id: `TODO-${Date.now()}-${index}-${i}`,
                                description: desc.trim(),
                                priority: 'medium',
                                assignee: c.participant || '未指定',
                                deadline: null,
                                status: 'pending',
                                meetingId: this.meetingId,
                                createdDate: new Date().toISOString()
            ...
            // 识别包含具体任务的内容
            const taskPatterns = [
                /(?:完成|做好|做好|推进|跟进|负责|落实)(.{5,30}?)(?:\n|。|$)/g,
                /(.+?)(?:完成|做好|做好|推进|跟进|负责|落实)/g
            ];
            ...
            taskPatterns.forEach(pattern => {
                ...
                while ((match = pattern.exec(text)) !== null) {
                    ...
                    if (desc.length > 5 && desc.length < 50) {
                        todos.push({
                            id: `TODO-${Date.now()}-${index}-${Math.random().toString(36).substr(2, 5)}`,
                            description: desc.trim(),
                            priority: 'medium',
                            assignee: c.participant || '未指定',
                            deadline: null,
    ...
    // 悬浮记录条功能
    toggleFloatingBar() {
        const bar = document.getElementById('floating-record-bar');
        const toggle = document.getElementById('floating-bar-toggle');
        
        if (bar.classList.contains('translate-y-full')) {
            bar.classList.remove('translate-y-full');
            toggle.innerHTML = '<i class="fas fa-chevron-down mr-2"></i>收起记录栏';
            toggle.classList.add('bottom-[100px]');
            toggle.classList.remove('bottom-4');
            document.getElementById('floating-host-input').focus();
        } else {
            bar.classList.add('translate-y-full');
            toggle.innerHTML = '<i class="fas fa-edit mr-2"></i>打开记录栏';
            toggle.classList.remove('bottom-[100px]');
            toggle.classList.add('bottom-4');
        }
    }
    
    floatingAddContent() {
        const input = document.getElementById('floating-host-input');
        const text = input.value.trim();
...
`;
    ...
    confirmEndMeeting() {
        ...
        // 保存到 recentMeetings（兼容）
        const recentHistory = JSON.parse(localStorage.getItem('recentMeetings') || '[]');
        const existingRecentIndex = recentHistory.findIndex(m => m.id === this.meetingId);
        if (existingRecentIndex >= 0) {
            recentHistory[existingRecentIndex] = { ...recentHistory[existingRecentIndex], ...meeting };
        } else {
            recentHistory.unshift(meeting);
        }
        localStorage.setItem('recentMeetings', JSON.stringify(recentHistory.slice(0, 50)));
        
        // 保存到 meetingsHistory（历史页面使用）
        const meetingsHistory = JSON.parse(localStorage.getItem('meetingsHistory') || '[]');
        const existingHistoryIndex = meetingsHistory.findIndex(m => m.id === this.meetingId);
        if (existingHistoryIndex >= 0) {
            meetingsHistory[existingHistoryIndex] = { ...meetingsHistory[existingHistoryIndex], ...meeting };
        } else {
            meetingsHistory.unshift(meeting);
        }
        localStorage.setItem('meetingsHistory', JSON.stringify(meetingsHistory.slice(0, 100)));
        
        // 同步到云端
        cloudSync.saveMeetingsHistory(meetingsHistory.slice(0, 100));
...
window.extractActionItems = () => window.meetingRoom?.extractActionItems();
